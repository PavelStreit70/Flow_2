...
#Balíček který umí otevřít xlsx i bez Javy
...
4. Soubory byly transformovány - následně je třeba je načíst v jednom setu. Bohužel transformace nebyla u jednoho souboru provedena - důsledek nezkušenosti při gatování?
...
6. Pořadí souborů by se mělo shodovat s tabulkou - tudíž bude nutné soubory přeuspořádat. Hodí se i případná kontrola, zda vše odpovídá a pořádí není rozhozené.

# V prvním kroku načíst názvy z datasetu
# Reorganizace tabulky aby byla v pořadí v jakém jsou FSC soubory
# Kontrola - pokud dostaneme 0 - znamená to, že vše se shoduje
...
8. Vygenerujeme graf, který nám říká, kolik buněk mají jednotlivé vzorky. Izotypové kontroly nebývají specificky barvené a i z jiných důvodů by měly obsahovat méně buněk (kratší čas analýzy, množství, které se jich pro analýzu přpravuje).

```{r}
plotCounts(PC, group_by = 'sample_id')

# nicméně se výše uvedené očekávání se nepotvrdilo - chyba při gatování? 
```
...
Další markery zájmu: CD9, CD44, CD59, CD63, CD70, CD71, CD81, CD95, CD97, CD166, CD201, SSEA-4, EpCAM. 

```{r, warning = FALSE}
plotDR(PC, dr = "TSNE", color_by = c("CD9", "CD44", "CD59", "CD70", "CD71", "CD97", "CD201"), ncol = 3)
```
...
any(is.na(uncorrected))
any(is.na(corrected))

colnames(uncorrected)
colnames(corrected)

evaluate_emd(uncorrected = uncorrected,
             corrected = corrected,
             cell_col = "condition",
             batch_col = "batch",
             markers = markers)

table(uncorrected$condition, uncorrected$batch)
table(corrected$condition, corrected$batch)
...
#Tím pádem nemáš žádný překryv nebo společné podmnožiny buněk mezi batch, což EMD (Earth Mover’s Distance) potřebuje, aby mohl správně spočítat vzdálenost mezi distribucemi před a po korekci.

#Je tam chyba, které EMD neumožní spočítat (nejsou tam zastoupeny obě skupiny), nevím, jak to řešit
...
24. Now, delete all the columns that DO NOT correspond to measured parameters or sample ID (for the FCS export in the next step). 

```{r}

```

25. Then, change the sample IDs to factor and display all the levels. 
...
26. Use the for loop below to export all FCS files from the corrected expression matrix (they should appear in the ~/outputs/integrated folder). 
...
27. Load the integrated FCS files back as a flowSet. 
...
28. Re-load the panel info as panel_integrated (from panel_integrated_data sheet in the metadata.xlsx file). 
...
29. Again, extract the order of the FCS files from the flowSet and use it for re-ordering of the rows in the sample annotation table. Then, double check if the order of the rows is the same as the order of frames in the flowSet. Caution, this time you're working with the column Filename_integrated.
...
```{r}
sum((names(dataset@frames) == sample_annotation$Filename_integrated) == FALSE)
```

30. Assemble another SingleCellExperiment filling in the code below (again beware, you want to use the Filename_integrated column from sample annotation now). 

```{r}
PB_integrated <- prepData(dataset, panel_integrated, sample_annotation,
...
31. Again, plot the numbers of cells for each sample in the integrated dataset. 
...
32. Perform t-SNE on integrated data, again taking all features and 2000 cells per sample. 
...
33. Perform UMAP on integrated data, again taking all features and 2000 cells per sample.
...
34. Perform FlowSOM clustering on integrated data, again using all cells, all features, 15 x 15 grid and aiming for 50 final metaclusters.
...
36. Visualize both t-SNE and UMAP reductions. Which one is better now?
...
37. Facet the t-SNE reduction by run ID and check for the batch effect correction. 
...
38. Perform the exploratory steps, faceting your dataset by CLL stage and coloring the dimred by all measured parameters. 
